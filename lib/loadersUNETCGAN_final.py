...
class RadioUNet_s(Dataset):
    """RadioMapSeer Loader for accurate buildings and no measurements (RadioUNet_c)
    if fix_samples = 655: we assume a fixed sample size of 1% of 256x256
    if fix_samples = 0 we randomly pick a number in (655x10,655)"""
    def __init__(self,maps_inds=np.zeros(1), phase="train",
                 ind1=0,ind2=0, 
                 dir_dataset="RadioMapSeer/",
                 numTx=80,                  
    ...
    def __getitem__(self, idx):
        ...
        #     ydk=np.delete(yk,v)
        #print(p,q)
        x = np.log10(np.sqrt(np.square(xk-p)+np.square(yk-q))).flatten()
        ...
        #print(x.shape)
        #print(y.shape)
        pop, _ = curve_fit(objective, xd, yd)
        theta,c = pop
        genImg = c - 10*theta*np.log10(np.sqrt(np.square(p-self.img)+np.square(q-self.img.T)))
